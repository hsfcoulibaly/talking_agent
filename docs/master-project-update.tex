...
A key design goal is evaluation completeness. Prior work on VLM pipelines often evaluates only the perception stage. This project contributes a two-track evaluation: automated F1 for Stage~1, and an LLM-as-judge scoring method for Stage~3 whose dimensions are grounded in Adams's theory of game storytelling~\cite{Adams2014}. Adams defines a good game story as an account of dramatically meaningful events that is credible, coherent, without undue repetition, and without arbitrary content, and identifies length, characters, degree of realism, and emotional richness as key quality factors. These criteria directly motivate the seven scoring dimensions used in this project's dialogue evaluation.
...
The script \texttt{src/dialogue\_evaluator.py} implements an automated judge using the Adams dimensions above. For each run, it loads \texttt{*\_scene\_graph.json} and \texttt{*\_npc\_dialogue.txt}, builds a structured prompt that cites the Adams framework, and calls a Gemini model configured as an expert evaluator scoring all seven dimensions on a 1--5 scale. The judge also returns a list of \textbf{hallucinated entities}---proper nouns present in the dialogue but absent from the scene graph. Results are saved to \texttt{evaluation/dialogue\_scores.csv} and \texttt{evaluation/dialogue\_scores.json}.

The story generation prompt in Stage~3 of the pipeline (\texttt{src/main\_pipeline.py}) was updated to align with these criteria: it now specifies a target length of 80--120 words, explicitly prohibits mention of game-UI elements, requires emotional expression, and forbids undue repetition.

\subsection{Human evaluation}

The template \texttt{evaluation/human\_scoring\_template.csv} is pre-populated with the three run IDs and columns for all seven Adams-derived dimensions. Human raters are provided the corresponding \texttt{.txt} files from \texttt{data/generated\_stories/} and asked to score each dialogue using the same 1--5 scale as the automated judge. The recommended study design uses 3--5 raters scoring all three samples, which allows inter-rater agreement to be computed and the automated scores to be validated against human judgment.

\section{Limitations and Future Work}

Frame order is user-supplied (e.g., filename order), not inferred from video timestamps. VLM mistakes propagate into the graph and dialogue; consolidation may over- or under-merge events. The current LLM-as-judge evaluation relies on the same Gemini model family used for generation, which may introduce evaluator bias; future work could use an independent judge model. The pipeline does not yet integrate live game APIs or native scene graphs from engines; extending to engine telemetry would tighten grounding further. The human evaluation study has not yet been conducted; completing it would allow direct comparison of automated and human scores across all seven Adams-derived dimensions. Finally, evaluation results reported here reflect the pipeline as of April 2026; re-running \texttt{src/dialogue\_evaluator.py} after any prompt changes will produce updated scores.
